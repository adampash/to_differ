...
(function() {
  ...
  App.Article = (function(_super) {
    ...
    Article.prototype.initialize = function() {
      this.bind('change', this.computedAttributes, this);
      this.set_last_version();
      return this.computedAttributes();
    };

    Article.prototype.set_last_version = function() {
      return this.set('last_selected_version', this.get('versions').length);
    };

    Article.prototype.computedAttributes = function() {
      this.set({
        first_version: this.get('versions')[this.get('first_selected_version') - 1],
        last_version: this.get('versions')[this.get('last_selected_version') - 1],
        total_versions: this.get('versions').length
      });
      return this.set({
        diffed_version: {
          text: differ.parse(this.first_text(), this.last_text()),
          title: differ.parse(this.get('first_version').title, this.get('last_version').title),
          numbers: [this.get('first_selected_version'), this.get('last_selected_version')]
        }
      });
    };

    Article.prototype.first_text = function() {
      return this.get('first_version').text;
    };

    Article.prototype.last_text = function() {
      return this.get('last_version').text;
    };
...
(function() {
  ...
  App.ArticleView = (function(_super) {
    ...
    ArticleView.prototype.tagName = 'div';

    ArticleView.prototype.initialize = function() {
      _.bindAll(this, 'render');
      _.bindAll(this, 'render_slider');
      this.template = _.template($('#item-template').html());
      this.$tiny = $('#tiny_article');
      this.slider_template = _.template($('#slider-template').html());
      this.listenTo(this.model, 'change', this.render_article);
      return this.render();
    };
    ...
    ArticleView.prototype.render_slider = function() {
      var view;
      view = this;
      this.$el.prepend(this.slider_template(this.model.toJSON()));
      this.$el.find('#slider').slider().on('slide', function(event) {
        return view.change_versions(event.value);
      });
      this.$el.find('.slider').width('100%');
      this.$el.find('.slider').height('70');
      return this;
    };

    ArticleView.prototype.render_article = function() {
      var index, match, missing_space_regex, text;
      if (this.$article) {
        this.$el.find('.article').remove();
        this.$tiny.empty();
      }
      this.$article = this.template(this.model.toJSON());
      text = this.$article.replace(/\$del\$/g, '<del>').replace(/\$\/del\$/g, '</del>').replace(/\$ins\$/g, '<ins>').replace(/\$\/ins\$/g, '</ins>').replace(/\$spacebar\$/g, ' ');
      ...
      this.$el.append(text);
      this.$tiny.append(text);
      merge_adjacent('del');
      merge_adjacent('ins');
      return this;
    };

    ArticleView.prototype.change_versions = function(values) {
    ...
    ArticleView.prototype.hello_world = function() {
