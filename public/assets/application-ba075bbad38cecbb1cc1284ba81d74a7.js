...
(function() {
  ...
  App.Article = (function(_super) {
    ...
    Article.prototype.computedAttributes = function() {
      this.set({
        first_version: this.get('versions')[this.get('first_selected_version') - 1].text,
        last_version: this.get('versions')[this.get('last_selected_version') - 1].text,
        total_versions: this.get('versions').length
      });
      return this.set({
        diffed_version: {
          text: differ.parse(this.get('first_version'), this.get('last_version')),
